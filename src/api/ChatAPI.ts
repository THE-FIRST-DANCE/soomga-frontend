...
export async function getRooms() {
  ...
  const res = await api.get<Room[]>('/chat') // 1. 채팅방 리스트 목록
  return res.data
}
...
export async function createRoom(data: { me: number; counterpart: number }) {
  ...
  const res = await api.post('/chat', postData)
  return res.data
}

// ! 4. 특정 채팅방의 정보
export async function getRoom(roomId: string) {
  const res = await api.get<Room>(`/chat/${roomId}`)
  4000
  return res.data
}
...
export async function storeMessage(roomId: string) {
  const res = await api.get(`/chat/${roomId}/messages/store`)
